...
grids_density <- group_by(core_may_census_all, year, grid) %>% 
  ...
  dplyr::summarise(spr_number = n_distinct (squirrel_id)) %>% 
...
saveRDS(grids_density, "output/auxilliary-data/spring-density.RDS")
...
saveRDS(SUKL_yearly_density, "output/auxilliary-data/SUKL_yearly_density.RDS")
